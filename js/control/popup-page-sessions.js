...
import {getPopupOverlay, getLocationLayer, getFuture} from '../util';
...
function showPager(featureHtml) {
  ...
  const sessions = getFuture() ? featureHtml.find('.session.future').hide() : featureHtml.find('.session').hide();
  featureHtml.find('.feature-org').hide();
  $(sessions.get(0)).parent().parent().show();
  $(sessions.get(0)).show();
    console.info(sessions.get(0))
  const pager = $('#popup-pager').data('sessions', sessions);
  if (sessions.length > 1) {
    pager.find('.at').html(1);
    pager.find('.end').html(sessions.length);
...
function pageSessions(event) {
  const pager = $('#popup-pager');
  const button = $(event.currentTarget);
  const sessions = pager.data('sessions').hide();
  const at = pager.find('.at');
  const index = at.html() - 1;
  const incriment = parseInt(button.attr('data-incriment'));
  $('#popup').find('.feature-org').hide();
  if (index + incriment < sessions.length && index + incriment > 0) {
    at.html(index + incriment + 1);
    $(sessions.get(index + incriment)).parent().parent().show();
    $(sessions.get(index + incriment)).show();
    console.info(sessions.get(index + incriment))
  } else {
    at.html(1);
    $(sessions.get(0)).parent().parent().show();
    $(sessions.get(0)).show();
...
export function createPopup(map) {
  ...
  $('#popup-pager .previous, #popup-pager .next').on('click', pageSessions);
}
